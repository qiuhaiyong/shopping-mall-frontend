  ...
  methods: {
    ...
    handler: throttle(function (type, disNum, cart) {
      ...
      this.$store.dispatch('detail/addOrUpdateShopCart', { skuId: cart.skuId, skuNum: disNum }).then(
        value => {
          // 加入购物车成功，则再调获取数据的接口重新获取数据
          this.getData()
        },
        error => {
          console.log('shocart---获取购物车数据失败')
        }
      )
    ...
    // 删除购物车中某一个产品
    deleteCartById(cart) {
      this.$store.dispatch('shopcart/deleteCartListBySkuId', cart.skuId).then(
        value => {
          this.getData()
        },
        error => {
    ...
    updateChecked(cart, event) {
      ...
      this.$store.dispatch('shopcart/updateCheckedById', { skuId: cart.skuId, isChecked }).then(
        value => {
          this.getData()
        },
        error => {
    ...
    allChecked(event) {
      if (this.cartInfoList.length > 0) {
        let isChecked = event.target.checked ? '1' : '0'
        this.$store.dispatch('shopcart/allChecked', isChecked).then(
          value => {
            this.getData()
          },
          error => {
